...
// time event
int interval = 200;//timer's interval
...
void parseInfo(int totalCount,float minX,float maxX,float minY,float maxY) {  
...
Place parsePlace(int z,float x,float y,String name,String co) {
  co="FF"+co;
  color c=unhex(co);
  return new Place(z, name, x, y, c);
}
...
public void draw() {
  ...
  //textSize(30);
  //fill(255, 255, 255);
  //text("Seoul electrocity", width - 1000, height - 710);
...
void calcZoom() {
  ...
  void drawType()  {
      ...
      fill(63,126,224);
      textSize(40);
      text("Seoul Electricity Use",width/2,height*0.1);     
  }